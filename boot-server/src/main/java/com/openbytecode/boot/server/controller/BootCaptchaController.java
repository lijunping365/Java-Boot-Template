...
import com.openbytecode.boot.server.dto.BootCaptchaRequest;
import com.openbytecode.boot.server.dto.BootCaptchaRespDTO;
...
public class BootCaptchaController {

    private final ImageCodeGenerator imageCodeGenerator;
    private final SmsCodeGenerator smsCodeGenerator;

    public BootCaptchaController(ImageCodeGenerator imageCodeGenerator, SmsCodeGenerator smsCodeGenerator) {
        this.imageCodeGenerator = imageCodeGenerator;
        this.smsCodeGenerator = smsCodeGenerator;
    }

    @PostMapping("/create/image")
    public Result<BootCaptchaRespDTO> createImageCode(@RequestBody @Valid BootCaptchaRequest request) {
    ...
    public Result<BootCaptchaRespDTO> createSmsCode(@RequestBody @Valid BootCaptchaRequest request) {
        BootCaptchaRespDTO openJobCaptchaRespDTO = new BootCaptchaRespDTO();
        CaptchaGenerateRequest captchaGenerateRequest = new CaptchaGenerateRequest();
        captchaGenerateRequest.setRequestId(request.getDeviceId());
        try {
    ...
    private BootCaptchaRespDTO convert(ImageValidateCode imageValidateCode) throws IOException {
